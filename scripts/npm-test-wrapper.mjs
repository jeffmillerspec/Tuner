...
function run(cmd, args) {
  return spawnSync(cmd, args, { stdio: ['ignore', 'pipe', 'pipe'], encoding: 'utf8', windowsHide: true, shell: false, cwd: root, env: { ...process.env, CI: 'true' } });
}
mkdirSync(join(root, 'tests', 'reports'), { recursive: true });
let out = '';
let code = 0;
const theme = run(process.execPath, ['--test', 'tests/theme-validate.test.mjs', 'tests/theme-shades.test.mjs', 'tests/theme-persist.test.mjs']);
...
const report = ['timestamp=' + new Date().toISOString(), 'exit_code=' + code, 'node_test=' + (theme.status ?? 'n/a'), 'vitest=' + (vitest.status ?? 'n/a'), 'smoke=' + (smoke.status ?? 'n/a'), '', out].join('\n');
